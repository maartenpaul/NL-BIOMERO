...
#
# Changes
# 10/22 - Add parsing of shebang for (Python) interpreter
# 10/22 - Add python processor selection for scripts
...
import signal
import uuid
from omero_ext import killableprocess
import subprocess
...
class ProcessI(omero.grid.Process, omero.util.SimpleServant):
    ...
    def command(self):
        ...
        """
        return self.interpreter.split(" ") + ["./script"]
...
class ProcessorI(omero.grid.Processor, omero.util.Servant):
    ...
    def __init__(self, ctx, needs_session=True, use_session=None,
        ...
        print(f"I HAVE HACKED INTO THE SYSSSSSTEEEEEMMMMM!!!!!11 \n{ctx}, \n{accepts_list}, \n{cfg}, \n{category}")
    ...
    def isProcessorMatch(self, scriptContext, file):
        sf = self.internal_session()
        script = sf.getScriptService().getScriptText(file.id.val)
        # self.logger.info(f"Match? \n{scriptContext}\n{file}\n{script}")
        try:
            ## UGLY HACK TO GET AROUND THE DYNAMIC INVOCATION
            # if "NSDYNAMIC" in script:
            #     return True
            
            # jobParams = sf.getScriptService().getParams(file.id.val)
            # self.logger.info(f"job params: {jobParams}")
            # if "GPU" in jobParams.namespaces:  
            
            # 1. Check environment exists          
            if not self.isLauncherAvailable(script):
                self.logger.info(f"Python environment not available")
                return False
            # 2. Check hardware match
            if not self.isGPUMatch(script):                
                self.logger.info(f"Hardware environment not available")
                return False
        except Exception as e:
    ...
    def isGPUMatch(self, script):
        require_GPU = self.isGPUScript(script)
        have_GPU = self.isGPUAvailable()
        self.logger.info(f"Checking GPU: required? {require_GPU} available? {have_GPU}")
        ## Strict matching
        # return require_GPU == have_GPU
        ## Loose matching
        if require_GPU:
            return have_GPU
        else:
    ...
    def isGPUAvailable(self):
        return os.getenv("OMERO_SERVER_GPU", 'False').lower() in ('true', '1', 't')       

    def isGPUScript(self, script):
        return "GPU" in script

    def isLauncherAvailable(self, script):
        self.logger.info(f"Checking launcher...")
        launcher = self.parse_shebang(script)
        try:
            check_launcher = subprocess.check_output(launcher.split(" ") + ["--version"])
        except Exception as e:
            self.logger.info(f"Issue with launcher: {launcher} --> {e}")
            return False
        self.logger.info(f"No issue with launcher: {check_launcher}")        
        return True

    @remoted
    def willAccept(self, userContext, groupContext, scriptContext, cb,
                   current=None):
        self.logger.info(f"Will I Accept?")
        self.logger.debug(f"Will I Accept? \n{userContext} \n{groupContext} \n{scriptContext} \n{cb}")
        userID = None
        ...
        if scriptID:
            try:
                ...
                valid = ((file is not None) 
                         and self.isProcessorMatch(scriptContext, file))
            except:
                self.logger.error(
        ...
        if valid == valid:
            ...
            self.logger.info(
                "Not valid, not calling isProxyAccepted: %s group:%s scriptjob:%s - Valid: %s",
                userID, groupID, scriptID, valid)
    ...
    def process(self, client, session, job, current, params, properties=None,
        ...
        try:
            ...
            launcher, ProcessClass = self.find_launcher(current, scriptText)
    ...
    def find_launcher(self, current, scriptText):
        launcher = ""
        process_class = ""
        if current.ctx:
            self.logger.info("Context: %s", current.ctx)
            launcher = current.ctx.get("omero.launcher", "")
            process_class = current.ctx.get(
                "omero.process", "omero.process.ProcessI")

        if not launcher:
            ## SHEBANG PARSING ADDED BY TTLUIK / AMC
            launcher = self.parse_shebang(scriptText)

        if not launcher:
            launcher = sys.executable
        self.logger.info("Default: %s", sys.executable)
